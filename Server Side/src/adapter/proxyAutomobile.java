...
public abstract class proxyAutomobile {
	...
	public void buildAutoWithProperty(Properties p) {
		FileIO file = new FileIO("filename");
		a1 = file.parsePropertyFile(p, "Properties");
		autoMap.put(a1.getMake() + a1.getModel(), a1);
	}
	...
	public String displayModels() {
		...
		while(it.hasNext()) {
			Automobile temp = (Automobile)it.next();
			models += (count + 1) + ". " + temp.getMake() + " " + temp.getModel() + "\n";
			count++;
		}
	...
	public Object selectAuto(Object n) {
		Collection c = autoMap.values();
		Iterator it = c.iterator();
		...
		while(it.hasNext()) {
			...
			if(num.equals(count + 1)) {
				...
				System.out.println("Count: " + (count + 1) + " is not the correct Automobile");
				System.out.println("The client's selection is: " + (Integer)n);
			}
			count++;
		}
		System.out.println("Sending null to client...");
		return null;
	}
